"""mInsight processor for image intelligence tracking.

This module contains the mInsight class which is a pure processor that:
- Processes entity versions for intelligence extraction
- Performs reconciliation of entity changes
- Tracks images in the image_intelligence table
- Operates in an idempotent, restart-safe manner

Database Access Pattern:
- All database operations are atomic with short-lived sessions
- Sessions are opened, used, and immediately closed
- Data is transferred via Pydantic models (EntityVersionData)
- No session passing between functions

The orchestration (run loop, MQTT, signal handling) is handled by the CLI (m_insight_worker.py).
"""
...
class mInsight:
    ...
    def process(self, data: EntityVersionData) -> bool:
        ...
        Database access is atomic - opens session, checks qualification, closes session,
        then opens new session for upsert if needed.
    ...
    def _enqueue_image(self, entity_version: EntityVersionData) -> None:
        ...
        print(f"image enqueued {entity_id} {md5}")
    ...
    def run_once(self) -> int:
        ...
        Trade-offs:
        - Pro: Short-lived sessions, no lock contention
        - Pro: Crash-safe at any point (each operation is atomic)
        - Pro: Better for long-running processes
        - Con: Slightly more database round-trips
        - Con: Theoretical race condition if version table changes between read and update
               (mitigated by version-first update strategy)
        ...
        max_transaction_id = max(
            (version.transaction_id for version in entity_deltas.values()),
