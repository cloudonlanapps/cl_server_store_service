...
from store.common import models, database
...
from cl_ml_tools.plugins.face_detection.schema import FaceDetectionOutput, DetectedFace, BBox, FaceLandmarks
...
class TestJobProcessing:
    ...
    async def test_job_submission_face_detection(self, test_db_session: Session):
        ...
        # Create entity
        entity = models.Entity(label="test.jpg", md5="md5_1", is_collection=False)
        ...
        service = JobSubmissionService(mock_compute)
        callback = AsyncMock()
        
        job_id = await service.submit_face_detection(entity.id, "/path/to/test.jpg", callback)
    ...
    async def test_face_detection_callback_success(
        ...
        mock_pysdk = PySDKRuntimeConfig(face_embedding_threshold=0.7)
        ...
        # Verify job record deleted (as per code logic: self.job_submission_service.delete_job_record)
    ...
    async def test_face_embedding_callback_new_person(
        ...
        handler = JobCallbackHandler(mock_compute, mock_qdrant, MagicMock(), mock_store_config, mock_pysdk)
    ...
    async def test_job_status_updates(self, test_db_session: Session):
        ...
        service = JobSubmissionService(MagicMock())
