        ...
        try:
            ...
            if not payload_str:
                ...
                asyncio.run_coroutine_threadsafe(
                    message_queue.put((entity_id, "CLEARED", {})), loop
                )
                return
            ...
            # Put in queue
            asyncio.run_coroutine_threadsafe(message_queue.put((entity_id, status, data)), loop)
        ...
        with patch("store.db_service.database.SessionLocal", side_effect=TestingSessionLocal):
            ...
            while (loop.time() - start_time) < TIMEOUT:
                try:
                    ...
                    try:
                        eid, status, payload = await asyncio.wait_for(
            ...
            assert "completed" in received_statuses, (
                f"Did not receive 'completed' status. Got: {received_statuses}"
            )
